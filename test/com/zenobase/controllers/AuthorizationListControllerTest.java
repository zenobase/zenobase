...
public class AuthorizationListControllerTest extends ControllerTestSupport {
	...
	public void testFindByPrincipal() {
		...
		Result result = call(Authorization.PRINCIPAL.getName(), user.asIdentity().toString(), 0, 10);
	...
	public void testFindAllAsSuperuser() {
		...
		Result result = call(null, null, 0, 10);
		assertThat(result).hasStatus(OK).hasContent(list.toJson());
	}
	...
	public void testBadLimit() {
		when(auth.current()).thenReturn(new Authorization(user.asIdentity()));
		Result result = call(Authorization.PRINCIPAL.getName(), user.asIdentity().toString(), 0, Integer.MAX_VALUE);
		assertThat(result).hasStatus(BAD_REQUEST);
	}

	private static Result call(String field, String value, int offset, int limit) {
		return callAction(com.zenobase.controllers.routes.ref.AuthorizationListController.find(field, value, offset, limit));
	}
}
