...
public class OuraStepsResultTest extends ResultTestSupport {
	...
	public void test() {
		...
		OuraStepsResult result = new OuraStepsResult(readObject("OuraStepsResultTest.json"), TESTER, "Steps", DateTimeZone.forID("Europe/Berlin"));
		List<Event> events = result.getEvents();
		assertThat(events).hasSize(3);

		assertThat(events.get(0).getValue(Event.TAG)).isEqualTo("Steps");
		assertThat(events.get(0).getValues(Event.TIMESTAMP)).containsSequence(dateTime("2018-12-02T00:00:00+01:00"), dateTime("2018-12-03T00:00:00+01:00"));
