...
public class ScopeFilterTest extends FilterTestSupport {
	...
	protected void configureFilters(HttpRouting.Builder routing) {
		routing.addFilter(new ScopeFilter(authContext));
	}
	...
	@Test
	public void testAuthenticatedFirstParty() {
		when(authContext.current(any())).thenReturn(new Authorization(new Identity("user-1")));
		ping();
	}

	@Test
	public void testAuthenticatedWithClient() {
		when(authContext.current(any())).thenReturn(
			new Authorization(new Identity("user-1"), new Identity("client-1"), "external")
		);
		ping();
	}
}
