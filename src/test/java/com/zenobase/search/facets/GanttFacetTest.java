...
public class GanttFacetTest extends FacetTestSupport {
	...
	public void setUp() {
		...
		e3 = newEvent("t2", "2012-05-15T09:00:00Z");
		e4 = newEvent("t2", null);
		e5 = newEvent("t3", null);
	}
	...
	public void testDefault() {
		...
		node.path(0).path("label").isEqualTo(e3.getValue(Event.TAG));
		node.path(0).path("first").isEqualTo(e3.getValue(Event.TIMESTAMP).toString());
		node.path(0).path("last").isEqualTo(e3.getValue(Event.TIMESTAMP).toString());
		...
		node.path(1).path("first").isEqualTo(e1.getValue(Event.TIMESTAMP).toString());
		node.path(1).path("last").isEqualTo(e2.getValue(Event.TIMESTAMP).toString());
	}
