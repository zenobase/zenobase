...
/**
 * OAuth2 credentials for the Google Health API. Mirrors {@link com.zenobase.tasks.google.GoogleCredentialsManager} but
 * uses the {@code googlehealth.*} scope set and a distinct credentials {@code TYPE} so a user can hold both legacy
 * Google Fit credentials and new Google Health credentials during the migration window.
 */
public class GoogleHealthCredentialsManager extends OAuthCredentialsManager {

	private static final Logger logger = LoggerFactory.getLogger(GoogleHealthCredentialsManager.class);

	public static final String TYPE = "google-health";

	@Inject
	public GoogleHealthCredentialsManager(
		CredentialsRepository integrations,
		@Named("google-health.api.key") String apiKey,
		@Named("google-health.api.secret") String apiSecret,
		@Named("web.hostname") String callbackUrl
	) {
		super(TYPE, integrations, new GoogleHealthApi(), apiKey, apiSecret, callbackUrl);
	}
	...
	private void reauthorize(OAuthCredentials credentials) {
		var request = new OAuthRequest(Verb.POST, "https://oauth2.googleapis.com/token");
