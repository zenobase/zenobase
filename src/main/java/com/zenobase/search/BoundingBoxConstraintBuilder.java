...
public class BoundingBoxConstraintBuilder extends ConstraintBuilder {
	...
	private Query build(Location topLeft, Location bottomRight) {
		...
		return Query.of(q -> q.geoBoundingBox(g -> g.field(getPath())
				.boundingBox(GeoBounds.of(gb ->
						gb.tlbr(TopLeftBottomRightGeoBounds.of(tlbr -> tlbr.topLeft(GeoLocation.of(gl -> gl.latlon(
										LatLonGeoLocation.of(ll -> ll.lat(tlLat).lon(tlLon)))))
								.bottomRight(GeoLocation.of(gl -> gl.latlon(
										LatLonGeoLocation.of(ll -> ll.lat(brLat).lon(brLon)))))))))));
	}
}
