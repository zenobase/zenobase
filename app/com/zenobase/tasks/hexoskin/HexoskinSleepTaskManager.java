...
public class HexoskinSleepTaskManager extends HexoskinTaskManagerSupport<HexoskinSleepTask> {
	...
	String getPath(HexoskinTaskSupport task) {
		return "/api/v1/range/?limit=100&order_by=start&rank=0&include_metrics=44,1039,2003&trainingroutine=12&start__gt=" + task.getStart();
	}

	@Override
	protected HexoskinResultSupport parse(Response response, HexoskinProfileResult profile, HexoskinSleepTask task) {
		return new HexoskinSleepResult(parseObject(response), task.getPrincipal(), task.getTag(), task.getZone(), profile.isMetric());
	}
}
