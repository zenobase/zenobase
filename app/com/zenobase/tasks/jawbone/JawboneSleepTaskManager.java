...
public class JawboneSleepTaskManager extends JawboneTaskManagerSupport {

	@Inject
	public JawboneSleepTaskManager(JawboneCredentialsManager credentialsManager) {
		super(JawboneSleepTask.TYPE, credentialsManager);
	}
	...
	public Task newTask(String bucketId, Identity principal, ObjectNode settings) {
		...
		return new JawboneSleepTask(bucketId, principal, tag, marker);
	}

	@Override
	public Command execute(Task task, OAuthCredentials credentials) {
		return execute(task.as(JawboneSleepTask.class), credentials);
	}

	private Command execute(JawboneSleepTask task, OAuthCredentials credentials) {
		...
		for (String path = "/nudge/api/v.1.1/users/@me/sleeps"; path != null;) {
