...
public class FitbitBurnTaskManager extends FitbitTaskManagerSupport<FitbitBurnTask> {
	...
	public FitbitBurnTask newTask(String bucketId, Identity principal, ObjectNode settings) {
		...
		String tag = Objects.firstNonNull(settings.path("tag").textValue(), "burn");
	...
	protected Command safeExecute(FitbitBurnTask task, OAuthCredentials credentials, Token token) {
		...
		if (task.isHourly()) {
			for (LocalDate date = fromDate; date.isBefore(syncDate); date = date.plusDays(1)) {
				try {
					OAuthRequest request = new OAuthRequest(Verb.GET, "https://api.fitbit.com/1/user/-/activities/calories/date/" + date + "/1d/15min.json");
					Response response = send(request, credentials);
					events.addAll(new FitbitBurnIntradayResult(parseObject(response), task.getTag(), task.getPrincipal(),date, profile.getTimezone()).getEvents());
			...
			if (!fromDate.isAfter(toDate)) {
				OAuthRequest request = new OAuthRequest(Verb.GET, "https://api.fitbit.com/1/user/-/activities/calories/date/" + fromDate + "/" + toDate + ".json");
				Response response = send(request, credentials);
				events.addAll(new FitbitBurnResult(parseObject(response), task.getTag(), task.getPrincipal(), profile.getTimezone()).getEvents());
			}
		}
