...
public class MicrosoftHealthCredentialsManager extends OAuthCredentialsManager {
	...
	private static final String TYPE = "microsoft";

	private final String callbackUrl;

	@Inject
	public MicrosoftHealthCredentialsManager(CredentialsRepository repository, @Named("microsoft.api.key") String apiKey, @Named("microsoft.api.secret") String apiSecret, @Named("oauth.hostname") String callbackUrl) {
		super(TYPE, repository, new MicrosoftHealthApi(), apiKey, apiSecret, callbackUrl);
	...
	private void reauthorize(OAuthCredentials credentials) {
		OAuthRequest request = new OAuthRequest(Verb.POST, "https://login.live.com/oauth20_token.srf");
		request.addBodyParameter("grant_type", "refresh_token");
		request.addBodyParameter("redirect_uri", buildCallback(callbackUrl, credentials));
